...
static	Handle:g_hCvarMPGameMode, Handle:g_hCvarModes, Handle:g_hCvarModesOff, Handle:g_hCvarModesTog, Handle:g_hCvarAllow,
		Handle:g_hCvarGlow, Handle:g_hCvarGlowCol, Handle:g_hCvarHeal, Handle:g_hCvarRandom, Handle:g_hCvarSafe, Handle:g_hCvarTemp,
		bool:g_bCvarAllow, g_iCvarGlow, g_iCvarGlowCol, g_iCvarHeal, g_iCvarRandom, g_iCvarSafe, g_iCvarTemp,
		Handle:g_hCvarDecayRate, Handle:g_hCvarGnomeRate, Float:g_fCvarDecayRate, Float:g_fCvarGnomeRate,
		Handle:g_hTimerHeal, Handle:g_hMenuAng, Handle:g_hMenuPos, bool:g_bLoaded, g_iPlayerSpawn, g_iRoundStart,
		g_iGnomeCount, g_iMap, g_iGnomes[MAX_GNOMES][2], g_iGnome[MAXPLAYERS+1], Float:g_fHealTime[MAXPLAYERS+1];
...
{
	g_hCvarAllow =		CreateConVar(	"l4d2_gnome_allow",			"1",			"0=插件关闭, 1=插件开启.", CVAR_FLAGS );
	g_hCvarGlow =		CreateConVar(	"l4d2_gnome_glow",			"200",			"0=关, 设置最大距离发光的侏儒.", CVAR_FLAGS );
	g_hCvarGlowCol =	CreateConVar(	"l4d2_gnome_glow_color",	"255 0 0",		"0=默认发光颜色. 三个值0 - 255之间用空格分开. RGB: 红 绿 蓝..", CVAR_FLAGS );
	g_hCvarHeal =		CreateConVar(	"l4d2_gnome_heal",			"1",			"允许侏儒治疗.", CVAR_FLAGS );
	g_hCvarModes =		CreateConVar(	"l4d2_gnome_modes",			"",				"打开插件在这些游戏模式, 用逗号分开 (不能有空格). (空白 = 全部).", CVAR_FLAGS );
	g_hCvarModesOff =	CreateConVar(	"l4d2_gnome_modes_off",		"",				"关掉插件在这些游戏模式, 用逗号分开 (不能有空格). (空白 = 没有).", CVAR_FLAGS );
	g_hCvarModesTog =	CreateConVar(	"l4d2_gnome_modes_tog",		"0",			"打开插件在这些游戏模式. 0=全部, 1=战役, 2=生存模式, 4=对抗, 8=清道夫.添加数字加起来.", CVAR_FLAGS );
	g_hCvarRandom =		CreateConVar(	"l4d2_gnome_random",		"-1",			"-1=全部, 0=没有. 否则随机选择这许多侏儒从地图配置产生.", CVAR_FLAGS );
	g_hCvarSafe =		CreateConVar(	"l4d2_gnome_safe",			"0",			"在一轮开始产生了侏儒: 0=关, 1=在安全室, 2=随即装备玩家.", CVAR_FLAGS );
	g_hCvarTemp =		CreateConVar(	"l4d2_gnome_temp",			"90",			"-1=添加临时的健康, 0=添加到主要的健康. 1和100之间的值创建一个机会给主要的健康, 否则将是临时健康.", CVAR_FLAGS );
	...
	RegAdminCmd("sm_gnome",			CmdGnomeTemp,		ADMFLAG_ROOT, 	"生成一个临时的侏儒在你的准心.");
	RegAdminCmd("sm_gnomesave",		CmdGnomeSave,		ADMFLAG_ROOT, 	"在你的准心产生一个侏儒并保存到配置.");
	RegAdminCmd("sm_gnomedel",		CmdGnomeDelete,		ADMFLAG_ROOT, 	"删除侏儒你指向和删除配置如果保存.");
	RegAdminCmd("sm_gnomekill",		CmdGnomeWipe,		ADMFLAG_ROOT, 	"移除所有从当前地图和侏儒删除它们从配置.");
	RegAdminCmd("sm_gnomeglow",		CmdGnomeGlow,		ADMFLAG_ROOT, 	"切换到使发光所有侏儒看到他们被放置.");
	RegAdminCmd("sm_gnomelist",		CmdGnomeList,		ADMFLAG_ROOT, 	"显示一个列表的位置和侏儒的总数.");
	RegAdminCmd("sm_gnometele",		CmdGnomeTele,		ADMFLAG_ROOT, 	"传送到一个侏儒 (用法: sm_gnometele <index: 1 to MAX_GNOMES>).");
	RegAdminCmd("sm_gnomeang",		CmdGnomeAng,		ADMFLAG_ROOT, 	"显示一个菜单来调整你的侏儒角度准星掠过.");
	RegAdminCmd("sm_gnomepos",		CmdGnomePos,		ADMFLAG_ROOT, 	"示一个菜单来调整你的侏儒起源准星掠过.");
}
