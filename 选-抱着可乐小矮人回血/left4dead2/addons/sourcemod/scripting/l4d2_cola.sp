...
public OnPluginStart()
{
	...
	g_hCvarGlow =		CreateConVar(	"l4d2_cola_glow",		"200",			"0=关, 设置最大距离发光的可乐.", CVAR_FLAGS );
	g_hCvarGlowCol =	CreateConVar(	"l4d2_cola_glow_color",	"255 0 0",		"0=默认发光颜色. 三个值0 - 255之间用空格分开. RGB: 红 绿 蓝.", CVAR_FLAGS );
	...
	g_hCvarRandom =		CreateConVar(	"l4d2_cola_random",		"-1",			"-1=全部, 0=没有. 否则随机选择这许多可乐从地图配置产生.", CVAR_FLAGS );
	g_hCvarRate =		CreateConVar(	"l4d2_cola_rate",		"1.5",			"玩家每多少秒治愈多少HP .", CVAR_FLAGS );
	g_hCvarSafe =		CreateConVar(	"l4d2_cola_safe",		"0",			"在一轮开始产生了可乐: 0=关, 1=在安全室, 2=随即装备玩家.", CVAR_FLAGS );
	g_hCvarTemp =		CreateConVar(	"l4d2_cola_temp",		"5",			"-1=添加临时的健康, 0=添加到主要的健康. 1和100之间的值创建一个机会给主要的健康, 否则将是临时健康.", CVAR_FLAGS );
	CreateConVar(						"l4d2_cola_version",	PLUGIN_VERSION, "愈合可乐的插件版本.", CVAR_FLAGS|FCVAR_REPLICATED|FCVAR_DONTRECORD);
	...
	RegAdminCmd("sm_cola",			CmdColaTemp,		ADMFLAG_ROOT, 	"生成一个临时的可乐在你的准心.");
	RegAdminCmd("sm_colasave",		CmdColaSave,		ADMFLAG_ROOT, 	"在你的准心产生一个可乐并保存到配置.");
	RegAdminCmd("sm_coladel",		CmdColaDelete,		ADMFLAG_ROOT, 	"删除可乐你指向和删除配置如果保存.");
	RegAdminCmd("sm_colawipe",		CmdColaWipe,		ADMFLAG_ROOT, 	"移除所有从当前地图和可乐删除它们从配置。.");
	RegAdminCmd("sm_colaglow",		CmdColaGlow,		ADMFLAG_ROOT, 	"切换到使发光所有可乐看到他们被放置.");
	RegAdminCmd("sm_colalist",		CmdColaList,		ADMFLAG_ROOT, 	"显示一个列表的位置和可口可乐的总数.");
	RegAdminCmd("sm_colatele",		CmdColaTele,		ADMFLAG_ROOT, 	"传送到一个可乐 (用法: sm_colatele <index: 1 to MAX_COLAS>).");
	RegAdminCmd("sm_colaang",		CmdColaAng,			ADMFLAG_ROOT, 	"显示一个菜单来调整你的可乐角度准星掠过.");
	RegAdminCmd("sm_colapos",		CmdColaPos,			ADMFLAG_ROOT, 	"显示一个菜单来调整你的可乐起源准星掠过.");
}
...
public Action:tmrStart(Handle:timer)
{
	...
	else if( g_iCvarSafe == 2 )
	{
		...
		if( client )
		{
			new entity = GivePlayerItem(client, "weapon_cola_bottles");
			if( entity != -1 )
